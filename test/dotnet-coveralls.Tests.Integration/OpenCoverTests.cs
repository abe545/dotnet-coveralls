...
namespace Dotnet.Coveralls.Tests.Integration
{
    public class OpenCoverTests
    {
        ...
        {
            var filePath1 = Path.Combine(RepositoryPaths.GetSamplesPath(), "opencover", "Sample1", "EmptyReport.xml");
            var filePath2 = Path.Combine(RepositoryPaths.GetSamplesPath(), "opencover", "Sample2", "SingleFileReport.xml");
            var results = DryRunCoverallsMultiModeWithInputFile(filePath1, filePath2);
        ...
        private static CoverageRunResults DryRunCoverallsWithInputFile(string inputFilePath)
        {
            return CoverallsTestRunner.RunCoveralls(
                $"--open-cover {inputFilePath} --dry-run --repo-token MYTESTREPOTOKEN");
        }

        private static CoverageRunResults DryRunCoverallsMultiModeWithInputFile(string inputFilePath1, string inputFilePath2)
        {
            return CoverallsTestRunner.RunCoveralls(
                $"--open-cover {inputFilePath1} --open-cover {inputFilePath2} --dry-run --repo-token MYTESTREPOTOKEN");
        }
    }
}